...
class ListProduct extends \WeltPixel\AjaxInfiniteScroll\Block\Product\ListProduct
{
    ...
    private function initializeProductCollection()
    {
        ...
        */
        ...
        //Restrict based on hide categories
        if(!$this->helper->getCustomerSession()->isLoggedIn()) {
//            // Filter hidden category products
//            $hideCategories = $this->helper->hideCategories();
//            if (count($hideCategories) > 0) {
//                $collection->addCategoriesFilter(['in' => $hideCategories]);
//            }
//            $collection->addAttributeToFilter('entity_id',0);
        }else {

            //Product collection fileter based on customer codes
            $erpCustomerCodes = $this->helper->getErpCustomerCodes();
//            $collection->addAttributeToSelect(Data::ERP_PRODUCT_CATEGORY_CODE);
//            if ($erpCustomerCodes[0] != "" && count($erpCustomerCodes) > 0) {
//                $collection->addAttributeToFilter(Data::ERP_PRODUCT_CATEGORY_CODE, ["in", $erpCustomerCodes]);
//            }else{
//                //$collection->addAttributeToFilter(Data::ERP_PRODUCT_CATEGORY_CODE, ["in", array('')]);
//                $collection->addAttributeToFilter('entity_id',0);
//            }
        }
