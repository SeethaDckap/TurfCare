...
namespace LeanSwift\Turfcare\Model\Adapter\Mysql\Aggregation\DataProvider;
...
//                    $select->where('product_category.product_id IN (?)', $allowedProductIds);
//                }else{
//                    $select->where('product_category.product_id IN (?)', array(''));
//                }
            }
        } else {
            $currentScopeId = $this->scopeResolver->getScope($currentScope)->getId();
