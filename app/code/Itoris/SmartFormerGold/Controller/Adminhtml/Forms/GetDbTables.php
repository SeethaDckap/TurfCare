...
class GetDbTables extends \Magento\Backend\App\Action {
    
    public function execute() {
        $res = $this->_objectManager->get('Magento\Framework\App\ResourceConnection');
        $con = $res->getConnection('read');
	    $prefix = $this->_objectManager->get('Magento\Framework\App\DeploymentConfig')->get('db/table_prefix');
	    $tables = $con->fetchCol("SHOW TABLES");
		$s=$prefix.'|';
		foreach ($tables as $value) $s .= $value.'|';
		$this->helper()->_echo($s);
    }
