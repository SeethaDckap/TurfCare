...
class LoadForm extends \Magento\Backend\App\Action {
    
    public function execute() {
        $fid = $this->getRequest()->getParam('fid', 0);
        if ($fid) {
            $form = $this->_objectManager->create('Itoris\SmartFormerGold\Model\Form')->load($fid);
            if ($form->getId()) {
                header ("content-type: application/json");
                $this->helper()->_echo($form->getForm());
            }            
        }
    }
