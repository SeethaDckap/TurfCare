...
namespace Itoris\SmartFormerGold\plugins\captcha\securimage;
...
class securimage {
  ...
  function captchacode() {
      //$_SESSION['sfg']['sec_code'] = $this->createCode();
  ...
  function image($sec_code) {
      //$this->code = $_SESSION['sfg']['sec_code'];
      $this->code = $sec_code;
      $this->doImage();
  }
    ...
    if ($this->use_gd_font == true) {
      ...
      for($i = 0; $i < $strlen; ++$i) {
        ...
        if ($this->use_multi_text == true) {
          ...
          if($this->use_transparent_text == true) {
            $font_color = imagecolorallocatealpha($this->im, hexdec("0x$r"), hexdec("0x$g"), hexdec("0x$b"), (int)$alpha);
          } else {
