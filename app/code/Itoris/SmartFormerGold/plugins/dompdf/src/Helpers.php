...
class Helpers
{
    ...
    public static function dompdf_debug($type, $msg)
    {
        ...
        if (isset($_DOMPDF_DEBUG_TYPES[$type]) && ($_dompdf_show_warnings || $_dompdf_debug)) {
            ...
            \Magento\Framework\App\ObjectManager::getInstance()->get('Itoris\SmartFormerGold\Helper\Data')->_echo(basename($arr[0]["file"]) . " (" . $arr[0]["line"] . "): " . $arr[1]["function"] . ": ");
            Helpers::pre_r($msg);
        }
    }
    ...
    public static function record_warnings($errno, $errstr, $errfile, $errline)
    {
        ...
        if ($_dompdf_show_warnings) {
            //echo $errstr . "\n";
            \Magento\Framework\App\ObjectManager::getInstance()->get('Itoris\SmartFormerGold\Helper\Data')->_echo($errstr . "\n");
        }
