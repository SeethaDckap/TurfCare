...
use DOMXPath;
use Itoris\SmartFormerGold\plugins\dompdf\src\Frame\Factory;
use Itoris\SmartFormerGold\plugins\dompdf\src\Frame\FrameTree;
use HTML5_Tokenizer;
use HTML5_TreeBuilder;
use Itoris\SmartFormerGold\plugins\dompdf\src\Image\Cache;
use Itoris\SmartFormerGold\plugins\dompdf\src\Renderer\ListBullet;
use Itoris\SmartFormerGold\plugins\dompdf\src\Css\Stylesheet;
 ...
 * the {@link Itoris\SmartFormerGold\plugins\dompdf\src\Adapter\CPDF} class provides a simple, stateless interface to
 * PDF generation.)  PDFLib support has now also been added, via the {@link
 * Itoris\SmartFormerGold\plugins\dompdf\src\Adapter\PDFLib}.
 *
 *
 * @package dompdf
 */
    ...
    */
    ...
    public function render()
    {
            ...
            foreach ($this->tree->get_frames() as $frame) {
                ...
                if ($frame->get_node()->nodeName == "dompdf_generated") {
                    ...
                    $decoded_string = preg_replace_callback("/\\\\([0-9a-fA-F]{0,6})/",
                        function ($matches) { return \Itoris\SmartFormerGold\plugins\dompdf\src\Helpers::unichr(hexdec($matches[1])); },
                        $style->content);
        ...
        if ($_dompdf_show_warnings && isset($_dompdf_warnings)) {
            ...
            \Magento\Framework\App\ObjectManager::getInstance()->get('Itoris\SmartFormerGold\Helper\Data')->_echo('<b>Dompdf Warnings</b><br><pre>');
            foreach ($_dompdf_warnings as $msg) {
                //echo $msg . "\n";
                 \Magento\Framework\App\ObjectManager::getInstance()->get('Itoris\SmartFormerGold\Helper\Data')->_echo($msg . "\n");
            }

            if ($canvas instanceof CPDF) {
                //echo $canvas->get_cpdf()->messages;
                \Magento\Framework\App\ObjectManager::getInstance()->get('Itoris\SmartFormerGold\Helper\Data')->_echo($canvas->get_cpdf()->messages);
            }
            //echo '</pre>';
            \Magento\Framework\App\ObjectManager::getInstance()->get('Itoris\SmartFormerGold\Helper\Data')->_echo('</pre>');
            flush();
        }
     ...
     *
     * @param string $size 'letter', 'legal', 'A4', etc. {@link Itoris\SmartFormerGold\plugins\dompdf\src\Adapter\CPDF::$PAPER_SIZES}
    ...
    /**
     * Return the underlying Canvas instance (e.g. Itoris\SmartFormerGold\plugins\dompdf\src\Adapter\CPDF, Itoris\SmartFormerGold\plugins\dompdf\src\Adapter\GD)
     *
     * @return Canvas
     */
