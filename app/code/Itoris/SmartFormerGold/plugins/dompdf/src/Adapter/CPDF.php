...
namespace Itoris\SmartFormerGold\plugins\dompdf\src\Adapter;

use Itoris\SmartFormerGold\plugins\dompdf\src\Canvas;
use Itoris\SmartFormerGold\plugins\dompdf\src\Dompdf;
use Itoris\SmartFormerGold\plugins\dompdf\src\Helpers;
use Itoris\SmartFormerGold\plugins\dompdf\src\Exception;
use Itoris\SmartFormerGold\plugins\dompdf\src\Image\Cache;
use Itoris\SmartFormerGold\plugins\dompdf\src\PhpEvaluator;

/**
 * PDF rendering interface
 *
 * Itoris\SmartFormerGold\plugins\dompdf\src\Adapter\CPDF provides a simple stateless interface to the stateful one
...
class CPDF implements Canvas
{
    ...
    public function __construct($paper = "letter", $orientation = "portrait", Dompdf $dompdf)
    {
        ...
        $this->_pdf = new \Itoris\SmartFormerGold\plugins\dompdf\lib\Cpdf(
            $size,
            true,
    ...
    public function __destruct()
    {
        foreach ($this->_image_cache as $img) {
            ...
            if ($this->_dompdf->getOptions()->getDebugPng()) \Magento\Framework\App\ObjectManager::getInstance()->get('Itoris\SmartFormerGold\Helper\Data')->_echo('[__destruct unlink ' . $img . ']');
    ...
    protected function _convert_gif_bmp_to_png($image_url, $type)
    {
        ...
        if (!function_exists($func_name)) {
            if (!method_exists("Itoris\SmartFormerGold\plugins\dompdf\src\Helpers", $func_name)) {
                throw new \Exception("Function $func_name() not found.  Cannot convert $type image: $image_url.  Please install the image PHP extension.");
            }
            $func_name = "\\Itoris\\SmartFormerGold\\plugins\\dompdf\\src\\Helpers::" . $func_name;
        }

        set_error_handler(array("\\Itoris\\SmartFormerGold\\plugins\\dompdf\\src\\Helpers", "record_warnings"));
    ...
    public function image($img, $x, $y, $w, $h, $resolution = "normal")
    {
        ...
        if ($debug_png) {
            \Magento\Framework\App\ObjectManager::getInstance()->get('Itoris\SmartFormerGold\Helper\Data')->_echo("[image:$img|$width|$height|$type]");
        }

        switch ($type) {
            case "jpeg":
                if ($debug_png) \Magento\Framework\App\ObjectManager::getInstance()->get('Itoris\SmartFormerGold\Helper\Data')->_echo('!!!jpg!!!');
            ...
            case "bmp":
                if ($debug_png) \Magento\Framework\App\ObjectManager::getInstance()->get('Itoris\SmartFormerGold\Helper\Data')->_echo('!!!bmp or gif!!!');
                // @todo use cache for BMP and GIF
                $img = $this->_convert_gif_bmp_to_png($img, $type);

            case "png":
                if ($debug_png) \Magento\Framework\App\ObjectManager::getInstance()->get('Itoris\SmartFormerGold\Helper\Data')->_echo('!!!png!!!');

                $this->_pdf->addPngFromFile($img, $x, $this->y($y) - $h, $w, $h);
                break;

            case "svg":
                if ($debug_png) \Magento\Framework\App\ObjectManager::getInstance()->get('Itoris\SmartFormerGold\Helper\Data')->_echo('!!!SVG!!!');

                $this->_pdf->addSvgFromFile($img, $x, $this->y($y) - $h, $w, $h);
                break;

            default:
                if ($debug_png) \Magento\Framework\App\ObjectManager::getInstance()->get('Itoris\SmartFormerGold\Helper\Data')->_echo('!!!unknown!!!');
        }
    }
