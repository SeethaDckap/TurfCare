 ...
 *
 * Itoris\SmartFormerGold\plugins\dompdf\src\Adapter\PDFLib provides a simple, stateless interface to the one
 * provided by PDFLib.
 *
...
class PDFLib implements Canvas
{
     ...
     * @var array;
     */
    static public $PAPER_SIZES = array(); // Set to Itoris\SmartFormerGold\plugins\dompdf\src\Adapter\CPDF::$PAPER_SIZES below.
     ...
     * Class constructor
     *
     * @param mixed $paper The size of paper to use either a string (see {@link Itoris\SmartFormerGold\plugins\dompdf\src\Adapter\CPDF::$PAPER_SIZES}) or
    ...
    public function stream($filename, $options = null)
    {
        ...
        if (self::$IN_MEMORY) {
            //echo $data;
            \Magento\Framework\App\ObjectManager::getInstance()->get('Itoris\SmartFormerGold\Helper\Data')->_echo($data);
        } else {
            // Chunked readfile()
            $chunk = (1 << 21); // 2 MB
            ...
            while (!feof($fh)) {
                //echo fread($fh, $chunk);
                \Magento\Framework\App\ObjectManager::getInstance()->get('Itoris\SmartFormerGold\Helper\Data')->_echo(fread($fh, $chunk));
            }
            fclose($fh);

            //debugpng
            if ($this->_dompdf->getOptions()->getDebugPng()) \Magento\Framework\App\ObjectManager::getInstance()->get('Itoris\SmartFormerGold\Helper\Data')->_echo('[pdflib stream unlink ' . $this->_file . ']');
    ...
    public function output($options = null)
    {
        ...
        if (self::$IN_MEMORY) {
            ...
            if ($this->_dompdf->getOptions()->getDebugPng()) {
                \Magento\Framework\App\ObjectManager::getInstance()->get('Itoris\SmartFormerGold\Helper\Data')->_echo('[pdflib output unlink ' . $this->_file . ']');
            }
            if (!$this->_dompdf->getOptions()->getDebugKeepTemp()) {
                unlink($this->_file);
            }
