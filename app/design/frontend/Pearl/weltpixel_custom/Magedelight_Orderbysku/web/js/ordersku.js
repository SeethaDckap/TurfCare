/* Remove accordian and tab class injection from Magedelight support team */
require([
    "jquery",
    "mage/url",
    "mage/mage",
    ...
    $(document).ready(function () {
        ...
        //TC-252 Enter key press action
        $(document).on("keypress",".sku-value", function(e){
                if (e.keyCode == 13) {
                    e.preventDefault();
                }
        });
        $('#submit-sku-form').click(function (e) {
             $('#product_addtocart_form_1').submit();
        });
        //TC-252 Enter key press action
        getSkuSuggestions();
        ...
        $(add_button).click(function (e) {
            ...
            $(wrapper).append('<div class="fields additional Item-' + x + '">' +
                ...
                </div> </div><div class="field qty required"><label class="label" for="id-items' + x + 'qty"><span>'+ $.mage.__("Qty") +'</span></label>\n\
                ...
                <div class="field product-price price-container-' + x + '" style="display: none;">\n\
                    <label class="label" for="id-items' + x + 'price"><span>' + $.mage.__("List Price") + '</span></label>\n\
                    <div class="obs-price product-price-' + x + '"></div>\n\
                </div>\n\
                <div class="field product-price customer-price-container-' + x + '" style="display: none;">\n\
                    <label class="label" for="id-items' + x + 'price"><span>' + $.mage.__("Your Cost") + '</span></label>\n\
                    <div class="customer-price product-customer-price-' + x + '"></div>\n\
                </div>\n\
                ...
                <div class="field obs-product-info-comments line-comments-' + x + '" style="display: none;">\n\
                    <label class="label" for="id-items0comments"><span>'+ $.mage.__("Comment :") + '</span></label>\n\
                    <div class="control">\n\
                        <input type="text" aria-required="true" class="product_comment" class="product_comment" name="items[' + x + '][product_comment]" id="id-items0productcomment" name=""></div>\n\
                    </div>\n\
        ...
        function getSkuSuggestions() {
            ...
            $(".sku-value").on("keyup", function (e) {

                var suggestionHtml;
                var itemnumber = $(this).attr('data-item');
                $('.Item-'+ itemnumber + '  .sku .control .error-message').remove();
                $('.Item-'+ itemnumber + ' .sku .control .sku-value').css("border","1px solid #c2c2c2");
                if ($(this).val() != '' && $(this).val().length > 2) {
                    ...
                    if (e.keyCode == 13) {
                            e.preventDefault();
                            var changedSku = $(this).val();
                            var changedAttr = $(this).attr('data-item');
                            getProductExistValidation(changedSku, changedAttr);
                            setTimeout(function(){
                            ...
                            $('.sku-suggestions').html('');
                            $('.sku-suggestions').hide();
                            $('.sku-suggestions-' + itemnumber - 1).html('');
                            $('.sku-suggestions-' + itemnumber - 1).hide();
                            changedAttr = changedSku = '';
                    }
                        ...
                        success: function (response) {
                            ...
                            if (response.html.size > 0) {
                                $(response.sku_array).each(function () {
                                    ...
                                    suggestionHtml += '<div class="sku">' + '<strong>' + $.mage.__('SKU# ')+'</strong>' + '<span class="sku-value-text">' +$(this)[0]['sku'] + '</span> </div>';
                                    if ($(this)[0]['supersedes']) {
                                        suggestionHtml += '<div class="supersedes-sku">' + '<strong>' + $.mage.__('OLD SKU# ') + '</strong>' + $(this)[0]['supersedes'] + '</div>';
                                    }
                                    suggestionHtml += '</div>';
                                    suggestionHtml += '</div>';
                                });
                                ...
                                $('.sku-suggestions-' + itemnumber).css("height", "200px");
                                 if(itemnumber > 0){
                                    $('.sku-suggestions-' + itemnumber - 1).hide();
                                    $('.sku-suggestions-' + itemnumber - 1).html('');
                                }
                                $('.sku-suggestions-' + itemnumber + ' .item').on('click', function () {
                                    $(this).parent().parent().find('.sku-value').val($(this).find('.sku-value-text').text());
                                    $(this).parent().parent().find('input.qty').focus();
                                    $(this).parent().parent().find('.sku-value').trigger('focusout');
                                    var textBoxValue = $(this).find('.sku-value-text').text();
                                ...
                                suggestionHtml += '<div class="messages">';
                                suggestionHtml += '<div class="sku-message">' + $.mage.__('No results') + '</div>';
                                suggestionHtml += '</div>';
                                $('.sku-suggestions-' + itemnumber).show();
                                $('.sku-suggestions-' + itemnumber).html(suggestionHtml);
                                $('.sku-suggestions-' + itemnumber).css("height", "20px");
                            }
                        }
                    });
                }
        ...
        function getProductDetails(productSku, itemnumber) {
            ...
            $('.Item-'+ itemnumber + ' .control .error-message').html("");
            $('.Item-'+ itemnumber + ' .control .error-message').css("color","none");
            $('.Item-'+ itemnumber + ' .control .sku-value').css("border","1px solid #c2c2c2;");
            ...
            currentRequest = $.ajax({
                ...
                success: function (data) {
                    if (data == '') {
                        ...
                        $('.product-customer-price-' + itemnumber).html('');
                        ...
                        $('.customer-price-container-' + itemnumber).hide();
                        $('.line-comments-' + itemnumber).hide();
                        $('.Item-'+ itemnumber + ' .sku .control .sku-value').css("border","1px solid red");
                        $('.Item-'+ itemnumber + '  .sku .control .error-message').remove();
                        $('.Item-'+ itemnumber + '  .sku .control').prepend("<span class='error-message'>"+ $.mage.__('Invalid sku') +"</p>");
                        $('.Item-'+ itemnumber + ' .sku .control .error-message').css("color","#c62421");
                        return true;
                    }else{
                        $('.Item-'+ itemnumber + '  .sku .control .error-message').remove();
                    }
                    ...
                    if (!$.isEmptyObject(data)) {
                        ...
                        if(typeof (data.customer_price_html !== undefined) && (data.price_without_cur !== data.customer_price )) {
                            $('.product-customer-price-' + itemnumber).html(data.customer_price_html);
                            $('.customer-price-container-' + itemnumber).show();
                        }else{
                            $('.customer-price-container-' + itemnumber).hide();
                        ...
                        $productViewUrl = '<a href="'+ data.product_url+'" target="_blank" class="view-detail-link">'+ $.mage.__('View Details') +'</a>';
                        $('.product-item-' + itemnumber).html($productViewUrl);
                        $(add_button).click();
                        $('.sku-suggestions-' + itemnumber).html('');
                        $('.sku-suggestions-' + itemnumber).hide();

                    } else {
                        $('.product-item-' + itemnumber).html('<span style="color: #e02b27; font-size: 1.2rem;" class="option-need-item">'+ $.mage.__('Please enter valid SKU key.')+'</span>');
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if (data.error === true) {
                            ...
                            $('.note-massege').html('');
                            $('.item-sku-mapping').append('<div class="itemgrid"><table class="itemtable"><tbody><tr><th class="item-name">'+ $.mage.__('Name')+'</th><th class="item-sku">'+ $.mage.__('Sku')+'</th><th class="item-qty">'+ $.mage.__('Qty')+'</th><th class="item-qty">'+ $.mage.__('Comment')+'</th><th class="item-massege" style="text-align:right">'+ $.mage.__('Message')+'</th></tr></tbody></table></div>');
                            $.each(data.maindata, function (key, value) {
                                var supersededList = value.superSededList;
                                if (value.invalidlist.length !== 0) {
                                    $.each(value.invalidlist, function (invalid, invalidvalue) {
                                        if(invalidvalue) {
                                            if(!supersededList[invalidvalue]) {
                                                $('.item-sku-mapping').append('<div class="itemgrid invalid-data-sku" data-role="collapsible"><div data-role="title" class="itemgrid-title "><table class="itemtable"><tbody><tr><td colspan="5" class="item-sku" style="color: #c62421 !important; font-size: 14px !important;">' + value.massege + ' : ' + invalidvalue + '</td></tr></tbody></table></div>');
                                            }else{
                                                $('.item-sku-mapping').append('<div class="itemgrid invalid-data-sku" data-role="collapsible"><div data-role="title" class="itemgrid-title "><table class="itemtable"><tbody><tr><td colspan="5" class="item-sku" style="color: #c62421 !important; font-size: 14px !important;">' + invalidvalue + $.mage.__('is replaced by')+ supersededList[invalidvalue] + '</td></tr></tbody></table></div>');
                                            }
                                        }
                                    });
                                } else {
                                    var productUrl = value.productUrl;
                                    configure = '<a data-key="' + key + '" data-sku="' + value.sku + '" data-option-url="' + optionUrl + '?sku=' + value.sku + '&qty=' + value.qty + '" class="options_selection" id="options_selection" href="' + productUrl + '">'+ $.mage.__('Configure')+'</a>';
                                    if (value.productwithOptionslist.length === 0) {
                                        options = false;
                                        configure = '';
                                    }
                                    $('.item-sku-mapping').append('<div class="itemgrid" id="' + value.typeid + '" data-role="collapsible"><div data-role="title" class="itemgrid-title "><table class="itemtable"><tbody><tr><td class="item-name">' + value.name + '</td><td class="item-sku"><input type="text" readonly name="items[' + key + '][sku]" value="' + value.sku + '" /></td><td class="item-qty"><input type="text" name="items[' + key + '][qty]" value="' + value.qty + '" /></td><td class="item-comments"><input type="text" readonly name="items[' + key + '][product_comment]" value="' + value.product_comment + '" /></td><td class="item-massege">' + value.massege + ' ' + configure + '<div data-key="' + key + '" id="product-options-wrapper-csv-' + key + '" class="product-options"></div></td></tr></tbody></table></div></div>');
                                }
                            });

                            $('.item-sku-mapping').append('<div class="actions-toolbar"><div class="primary"><button class="action tocart primary" title="Add to Cart" type="submit"><span>'+ $.mage.__('Add to Cart')+'</span></button></div></div>');
                            if (data.outstocklist.length !== 0) {
                                $('.csv-error-skus').append('<div class="out-of-stocklist" style="color: #e02b27; font-size: 1.2rem;">'+ $.mage.__('These sku are out of stock:')+' </div>');
                                $.each(data.outstocklist, function (key, value) {
                                    $('.csv-error-skus .out-of-stocklist').append(value + ';');
                                });
                            }
                            if (data.qtylist.length !== 0) {
                                $('.csv-error-skus').append('<div class="qty-list" style="color: #c62421; font-size: 1.2rem;">'+ $.mage.__('Product Not available:')+'</div>');
                                $.each(data.qtylist, function (key, value) {
                                    $('.csv-error-skus .qty-list').append(value + ';');
                                });
                            }
                            if (data.nonsimplelist.length !== 0) {
                                $('.csv-error-skus').append('<div class="non-simple-list" style="color: #c62421; font-size: 1.2rem;">'+ $.mage.__('These sku\'s are need to choose options:')+' </div>');
                                $.each(data.nonsimplelist, function (key, value) {
                                    $('.csv-error-skus .non-simple-list').append(value + ';');
                                });
                            }
                            if (data.invalidlist.length !== 0) {
                                $('.csv-error-skus').append('<div class="invalid-list" style="color: #c62421; font-size: 1.2rem;">'+ $.mage.__('The following skus have errors. Please see below for details:')+'</div>');
        ...
        function getProductExistValidation(productSku, itemnumber) {
            $('.Item-'+ itemnumber + ' .sku .control .sku-value').css("border","1px solid #c2c2c2;");
            $('.Item-'+ itemnumber + ' .control .error-message').html("");
            $('.Item-'+ itemnumber + ' .control .error-message').css("color","none");
            var ajaxUrl = urlBuilder.build('orderbysku/customer/product');
            ...
            currentRequest = $.ajax({
                ...
                success: function (data) {
                    ...
                    if (!$.isEmptyObject(data)) {
                        ...
                        $('.product-name-' + itemnumber).html(data.name);
                        $('.product-price-' + itemnumber).html(data.price_html);
                        if(typeof (data.customer_price_html !== undefined)) {
                            $('.product-customer-price-' + itemnumber).html(data.customer_price_html);
                            $('.customer-price-container-' + itemnumber).show();
                        }else{
                            $('.customer-price-container-' + itemnumber).hide();
